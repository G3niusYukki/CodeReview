...
def test_excel_adaptive_import_normalizes_full_province_names(tmp_path: Path) -> None:
    xlsx_path = tmp_path / "申通报价.xlsx"
    with ZipFile(xlsx_path, "w") as z:
        shared = ["始发地", "收件城市", "首重价格", "续重价格", "快递公司", "福建省", "广西壮族自治区", "4.0", "2.5", "申通快递"]
        z.writestr(
            "[Content_Types].xml",
            """<?xml version='1.0' encoding='UTF-8'?>
<Types xmlns='http://schemas.openxmlformats.org/package/2006/content-types'>
...
<worksheet xmlns='http://schemas.openxmlformats.org/spreadsheetml/2006/main'>
  <sheetData>
    ...
    assert result.records
    row = result.records[0]
    assert row.origin == "福建"
    assert row.destination == "广西"


def test_city_to_province_fallback_matches_city_level_tables(tmp_path: Path) -> None:
    repo = _make_repo_csv(tmp_path, "快递公司,始发地,目的地,首重,续重\n圆通,克拉玛依市,合肥市,3.0,2.0\n")
    rows = repo.find_candidates("克拉玛依", "安徽", courier="圆通")
    assert rows
    assert rows[0].origin == "克拉玛依市"
    assert rows[0].destination == "合肥市"
